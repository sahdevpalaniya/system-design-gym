  ...
  stages: [
    {
      ...
      model: [
        ...
        'Second question: on-demand only, or live streaming too? These are genuinely different systems — live has no processing slack at all, needs low-latency segment production, and cannot pre-transcode. I will assume on-demand, and say I am consciously excluding live because it would change the pipeline fundamentally.',
...

    {
      ...
      model: [
        ...
        'At transcoding: the worker crashes halfway through — the job retries, and because output is written per rendition to a deterministic path, a retry simply redoes work rather than corrupting anything. At transcoding: the file is genuinely broken or in an unsupported format — this is not retryable, so it must be distinguished from a transient failure and moved to a failed state with a real message to the uploader, rather than retried forever.',
