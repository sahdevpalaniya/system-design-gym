...
export const RIDE_MATCHING: Problem = {
  ...
  stages: [
...

    {
      ...
      model: [
        ...
        'Historical positions: the stream feeds an append-only store for analytics and dispute resolution. Deliberately separate, with no coupling to matching, so a problem in the analytics pipeline cannot slow down a match.',
...

    {
      ...
      model: [
        ...
        'Consistency per feature: driver positions are deliberately stale by seconds and that is fine. The candidate list is a best-effort snapshot. Trip state is strongly consistent and durable, because it decides who gets paid. Saying those three things separately is the answer — one blanket consistency statement would be wrong for two of the three.',
      ...
      tradeoffs: [
        ...
        },
        {
          ...
          cost: 'A genuinely vanished driver holds the trip for up to 30 seconds. Tightening it would cancel drivers who merely went through a tunnel.',
        },
      ],
      sayThis:
  ...
  architecture: {
    caption:
      'Live positions live in memory and are never durable. Trip state is durable and strongly consistent. The two are deliberately separate systems.',
    nodes: [
