...
export const FOLLOW_UPS: FollowUp[] = [
  /* ---------------- break the scale ---------------- */
  {
    ...
    trap: 'The trap is that "add more servers" is really correct for the stateless tier, which makes it a comfortable answer. But the stateless tier is never what breaks — the interviewer is asking about the part that cannot be cloned, and answering about the part that can tells them you have not looked.',
  },
  {
    id: 'scale-celebrity',
    category: 'break-scale',
  ...
  },
  {
    ...
    strong:
      '"Honestly, for a lot of systems one big machine is the right answer and I would say so. My estimate said 500 GB and 2,000 writes a second, which one machine handles comfortably — so size is not the reason. The reasons to distribute are: availability, because one machine means downtime during any restart or hardware failure, and I need a replica regardless; blast radius, because one machine means every customer shares one failure; and eventually write throughput, which I am nowhere near. So my actual design is one primary with a replica for failover, and I would revisit when writes approach 60% of what the primary can commit. Choosing not to distribute, on purpose, with a stated trigger for changing my mind, is the answer — distributing by reflex costs me every future feature."',
  ...
  },
  {
    ...
    weak: '"Postgres is very fast these days." Not a defence. 50,000 writes a second on a single primary is really beyond what one machine commits comfortably, and hand-waving it means you did not check.',
    strong:
  ...
  },
  {
    ...
    strong:
      '"For most of this I would not build. A managed queue, a managed database and a CDN are all things where the operational burden of running them myself vastly exceeds the licence cost, and running them badly is worse than paying. The cases where building wins: when the managed option cannot meet a specific requirement I can name — a latency budget, a data residency rule, a scale beyond what they offer; when the cost really crosses over, which happens at large scale for bandwidth and storage more than for compute; and when it is my actual product differentiator. For this system, the matching logic is the product and I would build that; the queue, the object storage and the CDN are commodity and I would buy. The cost of buying: vendor lock-in and a dependency whose outages are my outages, which I would mitigate by keeping the interface narrow enough to replace."',
  ...
  },
  {
    ...
    strong:
      '"First I need to be able to attribute load per customer, which means metrics tagged by tenant — a decision made in advance, not during the incident. Then immediate mitigation: tighten their rate limit, which must be per-key configuration so it is a config change rather than a deploy. Then the structural fix, which is isolation: separate connection pools or queues per tenant class so one customer cannot consume shared resources, which is the bulkhead pattern applied to tenancy. For a really large customer, dedicated capacity is often the right answer commercially as well as technically. What I would also check is whether they are abusing the system or using it as intended in a way I did not anticipate — because if it is the latter, rate limiting them is treating a product problem as an abuse problem, and the real fix might be a bulk API that does in one call what they are doing in ten thousand."',
    trap: 'Per-tenant attribution is the starting point and it has to exist before the incident. Without tagged metrics you know something is wrong and cannot tell who is causing it.',
  },
  ...
  },
  {
    ...
    trap: 'Backups and immutable archives are where this gets really hard, and the crypto-shredding answer is the one that shows you have dealt with it rather than read about it.',
  },
  {
    id: 'scope-offline',
    category: 'change-scope',
