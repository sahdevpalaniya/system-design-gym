...
export const RATE_LIMITER: Problem = {
  ...
  slack: {
    ...
    body: [
      ...
      'That asymmetry is unusual. Most systems in this app trade latency for accuracy in small amounts; here you can trade a lot of accuracy for a little latency, and you should say so out loud, on purpose.',
  ...
  stages: [
...

    {
      ...
      model: [
        ...
        'That reframes everything. This is not a data problem, it is a coordination problem: the state is easily small and the difficulty is entirely in keeping many machines roughly in agreement about it, cheaply.',
...

    {
      ...
      model: [
        ...
        'Security limits take a different path on purpose: login attempts go to a strongly consistent counter with an exact check, accepting the higher latency, because those are rare and being generous there has a real security cost. Two paths for two purposes, justified rather than uniform.',
  ...
  numbers: {
    ...
    note: 'A cross-region hop is 150x the entire budget, so global consistency is impossible by design here. So the hard part is deciding without coordinating — trade accuracy, which you have plenty of, for latency, which you have none of.',
  },
