...
export const TIER2: Concept[] = [
...

  {
    ...
    useWhen: [
      ...
      'The data really does not fit, or backups and recovery have become unmanageable.',
...

  {
    ...
    problem: [
      ...
      'CAP is that observation, and nothing more. It becomes really useful once you stop treating it as a slogan and start applying it to one operation at a time.',
    ],
...

  {
    ...
    cost: 'Every decision needs a round trip to a majority, so it is slow compared to a local write, and it gets slower the further apart the machines are. The cluster stops accepting writes if it cannot reach a majority, and that is on purpose. And it does not get faster with more machines. More machines means a bigger majority to convince, so a 7-node cluster is slower than a 3-node one.',
    useWhen: [
...

  {
    ...
    navTitle: 'Distributed transactions',
    tier: 2,
