  ...
  sections: [
...

    {
      ...
      body: [
        ...
        'It is genuinely fine when two things are true: every server is the same size, and every request costs about the same. In that world, taking turns spreads load perfectly and costs nothing to compute.',
...

    {
      ...
      points: [
        ...
        'Use when the fleet is deliberately heterogeneous — mixed instance sizes.',
...

    {
      ...
      body: [
        ...
        'For most real web traffic, where some requests are trivially fast and others are much slower, this is the sensible default. **Least response time** is a refinement that also weighs how quickly each server has been answering, which catches a server that is degraded but still accepting connections.',
      ],
      points: [
...
export const LOAD_BALANCING_EXAMPLE: WorkedExample = {
  ...
  steps: [
    ...
    },
    {
      step: 'Health checks',
      detail:
        'A `/healthz` endpoint that returns 200 if the process is up. It deliberately does not check Redis or the database — if the cache is down, redirect servers can still serve from the database, and I do not want the balancer removing every server because one dependency is slow.',
    },
    {
