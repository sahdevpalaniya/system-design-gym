  ...
  steps: [
    {
      ...
      detail:
        'Average CPU at 30% with slow queries means the work is not evenly distributed in time, or the queries are doing something structurally expensive. Since every partition is roughly equally busy, this is not a hot partition problem — which rules out the usual suspect immediately and points somewhere else.',
    },
    {
    ...
    {
      step: 'Handle the hot customer deliberately',
      detail:
        'For the few accounts big enough to saturate a partition, sub-partition by adding a bucket suffix — customer 419 becomes 419-0 through 419-7 — and fan reads across those eight. Those customers pay a small fan-out; everyone else keeps the single-partition read. Two paths, chosen deliberately, with a threshold tuned from measurement.',
    },
    {
