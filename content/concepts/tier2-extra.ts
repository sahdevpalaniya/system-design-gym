...
export const TIER2_EXTRA: Concept[] = [
  {
    ...
    body: [
      ...
      '**How to actually make the seat safe**, in order of preference. Best: let the database decide in one statement, with no read-then-write at all — `UPDATE seats SET taken = true WHERE id = ? AND taken = false`, then check how many rows changed. One row means you got it. Zero means someone else did. There is no gap for a race to live in. Next best: a unique constraint, so the second insert simply fails — that is how you stop duplicate usernames, and it works no matter how many application servers you run. After that: `SELECT ... FOR UPDATE` to lock the row, when you genuinely need to read and think before writing. Worst, and the one to name as wrong: read the value in application code, decide, then write.',
...

  {
    ...
    sayThis:
      '"I would not order these by wall clock, because two servers never agree and the loser is discarded silently. All writes for one conversation go through one partition, so the order is just arrival order. Where I genuinely need cross-machine ordering I would use a hybrid logical clock, so the value still looks like real time but the causal order is correct."',
...

  {
    ...
    useWhen: [
      ...
      'A single region is genuinely at capacity, which is rare and worth checking.',
    ],
    avoidWhen: [
    ...
    body: [
      ...
      '**Failover between regions** is not the same as failover between machines. The data in the failed region may be seconds behind, so promoting another region means accepting the loss of whatever had not replicated, and you should say how much that is. The decision also needs to be made by something outside both regions, or a network split leaves both believing they should be primary. And failing back afterwards is genuinely harder than failing over, because the recovered region now has writes that the promoted one never saw.',
    ...
    followUp: {
      ...
      answer:
        'My first question back is what kind of data, because the answer differs. For data that belongs to one user — their profile, their orders, their messages — I would give each user a home region based on where they signed up, write there, and replicate elsewhere for reads and disaster recovery. A European user gets a local write, no cross-Atlantic hop, and no conflict is possible because only one region ever writes their row. For data that is genuinely global and contested — a unique username, or worldwide inventory — I would keep a single authoritative region and accept that some users pay 80 to 100 milliseconds on those specific operations, because being right matters more than being fast on a rare action. The costs I would name: a routing layer that knows where each user lives, and a real migration story for the person who moves from New York to Berlin. And if this is European personal data I would raise residency as a constraint before latency, because that decides whether replicating to the US is even allowed — and it applies to backups and logs too, which is the part people miss.',
    },
    selfCheck: {
