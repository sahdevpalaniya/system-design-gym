...

  {
    ...
    visual: {
      ...
      compare: {
        ...
        verdict:
          'Use a cursor by default, and only offer offset paging where the product genuinely needs page numbers and the list is small. If someone asks for both, cursor is the API and page numbers are a UI built on top of a capped result.',
      },
    },
    body: [
    ...
    followUp: {
      ...
      answer:
        'The honest first answer is that I do not ship it as a break. Old app versions live on phones for years, some of those users will never update, and you cannot force them. So the default move is expand and contract: add the new field alongside the old one, populate both, ship the new app version reading the new field, watch usage of the old field decay, and only remove it when the number is small enough to accept the loss. That can take a year, and it is fine. When the change genuinely cannot be additive — the meaning of a field has changed, not just its name — then I version the endpoint, run both, and put a translation layer at the gateway so the old shape is produced from the new internal model rather than by maintaining two code paths. The costs I would name: two shapes to test, a translation layer that is easy to forget when adding features, and a deprecation date that I have to actually enforce with monitoring on old-version traffic, or v1 lives forever.',
    },
    selfCheck: {
