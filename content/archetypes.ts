...
export const ARCHETYPES: Archetype[] = [
    ...
    drill: [
      {
        ...
        strong:
          '"Because the constraint is not the database being slow, it is the read:write ratio. I estimated 4,000 reads a second against 40 writes, so the same small set of rows is being read repeatedly — a bigger instance costs more to serve the same rows over and over, while a cache serves them from memory once. A bigger instance is genuinely the right answer when the working set is large and access is uniform, which is not this case. Cost of my choice: a second system, stale data for the TTL window, and a stampede when it restarts, which I handle with coalescing."',
...

  {
    ...
    failing: [
      ...
      'You keep trying to zoom back out to the full architecture. They chose this box deliberately.',
